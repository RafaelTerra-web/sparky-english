...
- RewardState v5 usa bitset de propriedade e `store-ledger.ts` com posições permanentes. Migra arrays v1 e estados v3/v4, saldo, reembolsos, equipamentos e a sequência diária; não repete reembolso, check-in nem débito de compra já adquirida. A API pública continua fornecendo IDs legíveis.
