...
test('background lyrics never preview an unheard line and hold through instrumental gaps', () => {
  for (const [time, expected] of [[-1,-1],[0,-1],[2.999,-1],[3,0],[6,0],[9.999,0],[10,1],[18.999,1],[19,2],[30,2]]) {
...
test('background lyrics follow corrected media time after seeks, pauses and speed changes', () => {
  ...
  for (const speed of [1, .75, .5]) {
    ...
    assert.equal(ambientLyricIndex(lines, mediaTime), speed === 1 ? 1 : 0);
  ...
  assert.equal(ambientLyricIndex(lines, 10.1 - .2), 0);
});
