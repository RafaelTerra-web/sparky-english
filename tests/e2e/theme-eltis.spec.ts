...
test("ELTiS practice starts, keeps answers private and resumes", async ({ page }, testInfo) => {
  ...
  const confirm = page.getByRole("button", { name: "Confirmar e continuar" });
  ...
  await page.getByLabel("Open their textbooks").check();
  await page.getByLabel("Write two sentences").check();
  expect(answerSubmitted).toBe(false);
  await expect(page.getByLabel("Write two sentences")).toBeChecked();
  await expect(page.locator('.mock-question label.selected')).toContainText('Write two sentences');
  await confirm.click();
