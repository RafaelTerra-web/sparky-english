...
export type MusicProgress = { revision: number; version: string; stage: number; position: number; positionAt: number; heard: string[]; explored: string[]; saved: string[]; practiced: string[]; answers: Record<string, number>; completed: boolean };
export const emptyMusicProgress = (lesson: MusicLesson): MusicProgress => ({ revision: 0, version: lesson.version, stage: 0, position: 0, positionAt: 0, heard: [], explored: [], saved: [], practiced: [], answers: {}, completed: false });
...
export function normalizeMusic(lesson: MusicLesson, raw: unknown): MusicProgress {
  ...
  if (p.version !== lesson.version) return { ...blank, saved: ids(p.saved, words) };
  ...
  clean.completed = p.completed === true && canCompleteMusic(lesson, clean);
  return clean;
}
export function canCompleteMusic(lesson: MusicLesson, p: MusicProgress) { return p.stage === 4 && p.heard.length > 0 && p.explored.length > 0 && p.practiced.length > 0 && lesson.questions.every(q => p.answers[q.id] === q.answer); }
export function mergeMusic(lesson: MusicLesson, left: MusicProgress, right: MusicProgress): MusicProgress {
  ...
  return normalizeMusic(lesson, { ...newer, revision: Math.max(a.revision, b.revision), stage: Math.max(a.stage, b.stage), heard: [...a.heard,...b.heard], explored: [...a.explored,...b.explored], saved: [...a.saved,...b.saved], practiced: [...a.practiced,...b.practiced], answers, completed: a.completed || b.completed });
}
