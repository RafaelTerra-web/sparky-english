...
export default function MusicGame({ lesson, performance, onPerformance, onAchievements, onLyrics, media, clock, playing, speed, onSpeed, onSeek, onPlay, onPause, onExplore }: Props) {
  ...
  function start() { sound.current ??= createMusicFeedback(); sound.current.prepare(); setSeed(crypto.getRandomValues(new Uint32Array(1))[0]); submitted.current = null; setErrorFlash(null); setAchievementFlash(null); onSeek(0); dispatch({ type: 'start' }); setDraft({ index: -1, value: '' }); onPlay(); }
  ...
  return <section className="clip-game" aria-label="Jogo de escuta" data-session-seed={seed} data-phase={state.phase} data-playing={playing} data-difficulty={difficulty} data-tone={tone} data-section={section.kind} data-moment={moment} style={{ '--music-energy': energy } as CSSProperties}>
    ...
    {state.phase === 'ready' ? <div className="clip-setup">
      ...
      <div className="clip-prompt-slot">
        <MusicAmbience lines={lesson.lines} index={ambientIndex} clock={clock} focus={revealed ? { lineIndex: round.lineIndex, target: round.target, hidden: !state.answered } : undefined} />
      ...
      <div ref={status} className={'clip-feedback ' + (state.solved ? 'success' : state.roundMistakes ? 'retry' : '')} role="status" tabIndex={-1}>{achievementFlash ? `Conquista: ${t(achievementFlash)}` : state.feedback || (!playing ? 'Toque em continuar para voltar ao ritmo.' : heard ? 'Qual palavra você ouviu?' : revealed ? 'Acompanhe a voz…' : t(section.story))}</div>
      ...
      <div className={'clip-answer-receipt ' + (state.answered && !showingError && state.phase !== 'outro' ? 'is-visible ' : '') + (state.solved ? 'success' : '')} aria-hidden={!state.answered || showingError || state.phase === 'outro'}>{state.answered && <><span>{state.solved ? 'Acertou! +100 pontos' : 'A palavra era'}</span><strong lang="en">{round.answer}{state.solved && <Check size={20} />}</strong>{state.solved && <b key={state.index} className="clip-points-pop" aria-hidden="true">+100</b>}</>}</div>
