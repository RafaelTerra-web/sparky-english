...
export function ConversationTipCard({tip,mascot}:{tip:ConversationTip;mascot:'sparky'|'pinky'}) {
 ...
 return <aside className="conversation-tip" aria-label={`Dica de conversa: ${tip.title}`}>
  <div className="conversation-tip-heading"><MascotFigure mascot={mascot} equipped={{sparky:{},pinky:{}}} size="small" decorative/><div><p className="eyebrow">Dica de conversa · {mascot==='sparky'?'Sparky':'Pinky'}</p><h3>{tip.title}</h3></div></div>
  <p>{tip.tip}</p><blockquote lang="en">{tip.example}</blockquote><p className="tip-translation">{tip.translation}</p>
  <div className="speech-buttons"><button className="secondary-button" disabled={playing} onClick={()=>play(1)}>Ouvir dica</button><button className="secondary-button" disabled={playing} onClick={()=>play(0.75)}>Ouvir devagar</button>{playing&&<button className="secondary-button" onClick={stop}>Parar áudio</button>}</div>
  <p role="status">{error || (playing?'Reproduzindo a dica…':'')}</p>
  <p><strong>Experimente:</strong> {tip.tryIt}</p><details><summary>Conferir a ideia</summary><p>{tip.answer}</p></details>
  <details><summary>Texto da apresentação e referência</summary><p>{tip.script}</p><a href={tip.source} target="_blank" rel="noreferrer">Consultar referência de uso e pronúncia</a><p>Os exemplos e desafios são autorais. Os áudios são gerados por IA; sotaques e contextos podem variar.</p></details>
 </aside>;
}
