import { musicAchievements, musicDifficulties, musicRank, musicRanks, MUSIC_POINTS_PER_HIT, unlockedMusicAchievements, type MusicPerformance } from '@/lib/music-performance';
import { t } from '@/lib/interface-language';

const difficultyNames = { guided: 'Guiado', challenge: 'Desafio', typing: 'Sem pistas' };
export default function MusicAchievements({ performance }: { performance: MusicPerformance }) {
  ...
  return <section className="music-awards" aria-labelledby="music-awards-title">
    ...
    <div className="music-records">{musicDifficulties.map(mode => {
      const record = performance[mode], points = record.correct * MUSIC_POINTS_PER_HIT, rank = musicRank(points);
    ...
    <div className="music-rank-scale" aria-label={t('Pontos mínimos por rank')}>{musicRanks.map(rank => <span key={rank.name}><b>{rank.name}</b>{rank.points.toLocaleString('pt-BR')}</span>)}</div>
