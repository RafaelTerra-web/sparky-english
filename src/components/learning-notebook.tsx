"use client";
import { getInterfaceLocale, t, supportT, getSupportLocale } from "@/lib/interface-language";
...
export default function LearningNotebook({ userId, workspace, onOpen, themeId }: {
  ...
  return <div className={`learning-notebook ${theme?.className ?? ""}`}>
    ...
    {message && <p lang={getSupportLocale()} role="status" className="notice">{supportT(message)}</p>}
    <section className="profile-card notebook-section">
      ...
      <p lang={getSupportLocale()}>{supportT("Comece retomando sua prática, faça uma revisão pendente e avance quando couber no seu tempo. O nível escolhido no Perfil orienta o catálogo; não é um diagnóstico.")}</p>
    </section>
    <section className="profile-card notebook-section">
      <h2>{t("O que sua prática mostra")}</h2>
      <p lang={getSupportLocale()}>{supportT(workspace.attempts.length)}{supportT(" tentativas registradas neste dispositivo. Considerando a resposta mais recente de cada exercício:")}<strong>{supportT(independent)}{supportT(" acertos sem ajuda")}</strong>{supportT(" e")}<strong>{supportT(practice.length)}{supportT(" exercícios para praticar novamente")}</strong>.</p>
      <p lang={getSupportLocale()}>{supportT("Consultar uma explicação faz parte de aprender. Depois, tente responder sem ajuda para conferir o que lembra. Estes registros não avaliam seu nível de inglês, sua escrita ou sua pronúncia.")}</p>
      <h3>{t("Exercícios para rever")}</h3>
      {!practice.length && <p lang={getSupportLocale()}>{supportT("Os exercícios em que você errar ou consultar uma explicação aparecerão aqui para praticar novamente.")}</p>}
      <div className="notebook-list">{practice.slice(-20).reverse().map(attempt => {
        ...
        return lesson && <article key={attempt.stepId}>
          <h4>{t(lesson.title)}</h4><p lang={getSupportLocale()}>{supportT(attempt.correct ? "Acerto com ajuda" : "Resposta para rever")} · {supportT(new Date(attempt.createdAt).toLocaleDateString(getInterfaceLocale()))}</p>
          <details><summary>{t("Consultar sua resposta e a explicação")}</summary><p lang="en">{attempt.answer}</p><p lang={getSupportLocale()}>{supportT(lesson.steps.find(s => `${lesson.id}:${s.kind}` === attempt.stepId)?.explanation)}</p></details>
          <button className="secondary-button" onClick={() => onOpen(lesson, true)}>{t("Praticar novamente")}</button>
        </article>;
      })}</div>
    </section>
    <section className="profile-card notebook-section">
      <h2>{t("Frases para usar")}</h2><p lang={getSupportLocale()}>{supportT("Guarde exemplos durante as lições. Tente lembrar o significado antes de revelar a tradução.")}</p>
      {!workspace.vocabulary.length && <p lang={getSupportLocale()}>{supportT("Seu caderno de frases está vazio.")}</p>}
      <div className="notebook-list">{workspace.vocabulary.map(item => <article key={item.id}><p lang="en">{item.english}</p><details><summary>{t("Ver significado")}</summary><p lang="pt-BR">{item.translation || "Sem tradução cadastrada."}</p></details></article>)}</div>
    </section>
    <section className="profile-card notebook-section">
      <h2>{t("Escrita e novas versões")}</h2><p lang={getSupportLocale()}>{supportT("Seus textos ficam salvos neste dispositivo. Releia o que escreveu e revise a clareza, a estrutura e o vocabulário. Esta revisão não gera uma nota automática.")}</p>
      {!workspace.writings.length && <p lang={getSupportLocale()}>{supportT("As produções escritas das lições aparecerão aqui.")}</p>}
