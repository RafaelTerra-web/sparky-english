...
function MusicSession({ userId, lesson, lab, onClose }: { userId: string; lesson: MusicLesson; lab: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (paintAt - lastPaint >= 32 || current === studyEnd) { setTime(current); lastPaint = paintAt; }
