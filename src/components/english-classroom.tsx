...
export default function EnglishClassroom({level}:{level:Level}) {
 ...
 <audio key={id} controls preload="none" src={'/audio/classes/'+id+'.mp3'} onPlay={event=>claimAudioPlayback(event.currentTarget)} onEnded={event=>releaseAudioPlayback(event.currentTarget)} onError={event=>{releaseAudioPlayback(event.currentTarget);setError(true);}} aria-label={'Listen to '+lesson.title}/>
