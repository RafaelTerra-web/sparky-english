...
function Mission({ data, userId, packId }: { data: StoreMission; userId: string; packId: string }) {
  ...
  return <article className="store-mission"><h3 ref={heading} tabIndex={-1}>{t(data.title)}</h3>
    ...
    <section className="mission-writing"><h4>{t("Agora use suas palavras")}</h4><p lang={getSupportLocale()}>{supportT(data.writing)}</p>
      ...
      <p lang={getSupportLocale()} role={saveError ? "alert" : undefined}>{supportT(saveError ? "O navegador não conseguiu salvar. Copie seu texto antes de sair." : "Rascunho salvo neste navegador. Você pode fechar e retomar esta missão depois.")}</p>
...
export function StorePractice({ packId, userId, onClose }: { packId: string; userId: string; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="store-practice-dialog" aria-labelledby="store-practice-title" onCancel={event => { event.preventDefault(); onClose(); }}>
    <header><div><p className="eyebrow">{t("Missões extras ·")}{t(pack.level)}</p><h2 id="store-practice-title">{t(pack.name)}</h2></div><button className="icon-button" aria-label={localizeAttribute("Fechar missões")} onClick={onClose}><X size={20} /></button></header>
    <div className="mission-tabs" role="group" aria-label={localizeAttribute("Escolher missão")}>{missions.map((mission, i) => <button key={mission.id} aria-pressed={i === index} onClick={() => setIndex(i)}>{t(i + 1)}. {t(mission.title)}</button>)}</div>
